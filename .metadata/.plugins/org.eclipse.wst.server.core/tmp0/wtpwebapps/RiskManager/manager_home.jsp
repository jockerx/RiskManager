...
<%
	...
	pList = (List<ProductPlan>) ServletActionContext.getRequest().getAttribute("allPlanToManager");
	List<Member> mList;
	mList = (List<Member>) ServletActionContext.getRequest().getAttribute("allMember");
%>
<!--bootstrap-->
	...
	<div class="our-products" style="border-top: 3px solid pink;">
		<div class="container">
			<div class="products-gallery">
				<h2>PRODUCTPLANS TO BE APPROVED</h2>
				<div class="col-md-12 arr">
					<%
						int f = 0;
						while (pList.size() >= (f + 1)) {
							if(pList.get(f).getState()==0){
					%>
					<div class="col-md-12 arr" <%if (f > 0) {%> style="margin-top: 2%;"
						...
						<div class="bg">
							...
							<div style="text-align: left; padding: 20px 20px">
								...
								<p style="line-height: 3; font-size: 16px;">
									预计销售数量：<%=pList.get(f).getNum()%></p>
								<p style="line-height: 3; font-size: 16px;">
									预计销售单价：<%=pList.get(f).getPrice()%><a href="/SSH/approvalPlan.action?id=<%=pList.get(f).getId()%>" class="btn btn-primary" style="float:right">批准计划</a></p>
								<p style="line-height: 3; font-size: 16px;">
									预计销售店铺id：<%=pList.get(f).getStore_id()%><a  href="/SSH/rejectPlan.action?id=<%=pList.get(f).getId()%>" class="btn btn-primary" style="float:right">驳回计划</a></p>
							</div>
						</div>
					</div>
					<%}
