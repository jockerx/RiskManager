...
<body>
 ...
 %>
								...
								<li><a href="member_myinfo.jsp" class="active">MYINFO</a></li>
				...
				<div class="col-md-12 arr">
					<table class="table table-striped">
						...
						<tbody style="text-align: center; vertical-align: middle;">
							...
							<tr>
								<td>暂停时间</td>
								<td><%=m.getSuspendDate()%></td>
							</tr>
							<tr>
								<td>VIP等级</td>
								<td>
									<div class="progress" style="margin-left: 10%; width: 85%">
										<div class="progress-bar progress-bar-primary"
											style="width: 
								  <%if (m.getVipLevel() == 0) {%>0%;background-color:#337ab7;
						  <%} else if (m.getVipLevel() == 1) {%>20%;background-color:#337ab7;
						  <%} else if (m.getVipLevel() == 2) {%>40%;background-color:#4FD5A9;
						  <%} else if (m.getVipLevel() == 3) {%>60%;background-color:#98F780;
						  <%} else if (m.getVipLevel() == 4) {%>80%;background-color:#E2FF8A;
						  <%} else {%>100%;background-color:#FFC459;
						  <%}%>">
										</div>
									</div> VIP<%=m.getVipLevel()%>
								</td>
							</tr>
							<tr>
								<td>居住地</td>
							...
							<tr>
								...
								<td>
									<button type="button" class="btn btn-primary"
										style="float: right; margin-right: 30px;" data-toggle="modal"
										data-target="#updateInfo">更新个人信息</button>
									<button type="button" class="btn btn-primary"
										style="float: right; margin-right: 30px;" data-toggle="modal"
										data-target="#pointsToCard"">积分兑换</button>
								</td>
							</tr>
						</tbody>
					</table>
	...
	<!-- 模态框（Modal） -->
	<div class="modal fade" id="pointsToCard" tabindex="-1" role="dialog"
		aria-labelledby="myModalLabel" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
				...
				<form action="Member_pointsToCard">
					<div class="modal-body">
						请输入用于兑换的积分数(每100积分将兑换1元进入银行卡) <input type="text" class="form-control"
							placeholder="需为100的倍数" name="points"
							style="margin: 45px; width: 400px;">
					</div>
					<div class="modal-footer">
						<button type="button" class="btn btn-default" data-dismiss="modal">取消
						</button>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<form action="Member_update">
					<div class="modal-body">
						请输入姓名 <input type="text" class="form-control"
							placeholder="" name="name" value=<%=m.getName() %>
							style="margin: 20px 60px; width: 400px;">
						请输入年龄 <input type="text" class="form-control"
							placeholder="需为整数" name="age" value=<%=m.getAge() %>
							style="margin: 20px 60px; width: 400px;">
						请输入居住地 <input type="text" class="form-control"
							placeholder="" name="address" value=<%=m.getAddress() %>
							style="margin: 20px 60px; width: 400px;">
					</div>
					<div class="modal-footer">
						<button type="button" class="btn btn-default" data-dismiss="modal">取消
